...
**文件**: `src/regreader/storage/models.py`
...
**文件**: `src/regreader/parser/page_extractor.py`
...
# 重新入库
regreader ingest -f /path/to/2024年国调直调安全自动装置调度运行管理规定（第二版）.pdf \
                 -r angui_2024 \
                 --format pdf

# 验证结果
regreader inspect angui_2024 7
```
...
```bash
# 完整流程测试
regreader ingest -f data/test.pdf -r test_2024 --format pdf

# 验证文档结构
regreader inspect test_2024 7

# 验证搜索过滤
regreader search "母线失压" --reg-id test_2024 --block-types text,section_content

# 验证章节查询
regreader search "安控装置" --reg-id test_2024 --section-number "2.1.4.1.6"
```
