# GridCode CLI MCP 模式实现设计文档

## 概述

为 GridCode CLI 添加 MCP 访问模式，让所有业务命令都能通过 MCP Server 访问数据，以便全面验证 MCP 功能可用性，支撑后续智能体调用。
...
┌─────────────────────────────────────────────────────────────────┐
│                    create_tools() 工厂函数                       │
│          (use_mcp, transport, url) → GridCodeToolsProtocol       │
└────────────────────┬───────────────────────┬────────────────────┘
                     │                       │
         use_mcp=False                use_mcp=True
                     ...
                     ▼                       ▼
┌────────────────────────────┐   ┌────────────────────────────────┐
│    GridCodeTools           │   │   GridCodeMCPToolsAdapter      │
│    (本地直接访问)           │   │   (MCP 远程调用封装)            │
└────────────────────────────┘   └────────────────────────────────┘
```
...
```
命令行参数 --mcp > 环境变量 GRIDCODE_USE_MCP > 配置文件 use_mcp_mode > 默认值(False)
```
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `src/grid_code/config.py` | 修改 | 添加 MCP 模式配置项 |
| `src/grid_code/mcp/protocol.py` | 新增 | 工具协议接口定义 |
| `src/grid_code/mcp/adapter.py` | 新增 | MCP 工具适配器 |
| `src/grid_code/mcp/factory.py` | 新增 | 工具工厂函数 |
| `src/grid_code/mcp/client.py` | 修改 | 添加 SSE 传输支持 |
| `src/grid_code/mcp/__init__.py` | 修改 | 导出新增模块 |
| `src/grid_code/cli.py` | 修改 | 添加全局 MCP 选项 |
...
- 定义 `GridCodeToolsProtocol` Protocol 类
- 包含所有 15 个工具方法签名
...
gridcode toc -r angui_2024

# 2. MCP stdio 模式（自动启动子进程）
gridcode --mcp search "母线失压" -r angui_2024
gridcode --mcp read-pages -r angui_2024 -s 1 -e 5
...
# 4. 环境变量方式
export GRIDCODE_USE_MCP=1
export GRIDCODE_MCP_TRANSPORT=stdio
gridcode search "母线失压" -r angui_2024
```
