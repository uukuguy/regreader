...
`get_toc` 和 `get_chapter_structure` 是 GridCode 系统中两个用于获取规程结构信息的 MCP 工具。虽然它们都返回章节相关信息，但在设计意图、数据结构和使用场景上有明显区别。
...
**文件位置**: `src/grid_code/mcp/tools.py`
...
```bash
gridcode toc angui_2024 --expand --level 3
```
...
**文件位置**: `src/grid_code/cli.py` (第 638-849 行)

**命令签名**:
```bash
gridcode toc <reg_id> [OPTIONS]
...
```bash
# 默认显示 3 层
gridcode toc angui_2024

# 展开所有层级
gridcode toc angui_2024 --expand

# 只显示 2 层
gridcode toc angui_2024 --level 2

# 导出为 JSON
gridcode toc angui_2024 --output toc.json
```

### `chapter-structure` 命令

**文件位置**: `src/grid_code/cli.py` (第 888-935 行)

**命令签名**:
```bash
gridcode chapter-structure <reg_id> [OPTIONS]
...
```bash
# 显示章节结构
gridcode chapter-structure angui_2024

# 导出为 JSON
gridcode chapter-structure angui_2024 --output structure.json
```
...
**文件位置**: `src/grid_code/mcp/tool_metadata.py`
...
**文件位置**: `src/grid_code/storage/models.py`
...
| 文件 | 说明 |
|------|------|
| `src/grid_code/mcp/tools.py` | 工具实现代码 |
| `src/grid_code/mcp/tool_metadata.py` | 工具元数据定义 |
| `src/grid_code/cli.py` | CLI 命令实现（行 638-935） |
| `src/grid_code/storage/models.py` | TocTree 和 DocumentStructure 数据模型 |
| `src/grid_code/storage/page_store.py` | 数据加载逻辑 |
