# GridCode 页面数据对比脚本实现计划

## 任务概述

创建一个 CLI 命令 `grid-code inspect`，用于对比显示指定页面在三种数据源中的原始数据：
1. FTS5 关键词索引
2. LanceDB 向量索引
...
**文件**: `src/grid_code/services/inspect.py`（新建）
...
**文件**: `src/grid_code/services/inspect.py`
...
**文件**: `src/grid_code/services/inspect_display.py`（新建）
...
**处理方式**：
- 捕获异常并显示友好的错误消息
- 对于索引未构建的情况，提示用户先运行 `grid-code ingest`
...
| 组件 | 文件路径 | 说明 |
|------|---------|------|
| CLI 命令 | `src/grid_code/cli.py` | 新增 `inspect` 命令 |
| 核心服务 | `src/grid_code/services/inspect.py` | 新建，数据获取和分析 |
| 显示模块 | `src/grid_code/services/inspect_display.py` | 新建，终端显示格式化 |
| 数据模型 | `src/grid_code/storage/models.py` | 已存在，可能需要导入 |
| 配置 | `src/grid_code/config.py` | 读取索引路径配置 |
