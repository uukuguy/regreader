...
**设计原则**：
- 三个实现共享同一套 MCP Server 工具层
- **所有 Agent 必须通过 MCP 协议访问页面数据**（页面存储由 MCP Server 内部控制）
- 统一使用 `GridCodeMCPClient` 作为 MCP 客户端
...
│                      Agent 实现                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ ClaudeAgent  │  │ LangGraph    │  │ PydanticAI   │       │
│  │ (SDK 内置    │  │ Agent        │  │ Agent        │       │
│  │  MCP 支持)   │  │              │  │              │       │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘       │
│         │                 │                 │                │
...
│                   GridCode MCP Server                        │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  Tools: get_toc, smart_search, read_page_range, ...  │   │
│  └──────────────────────────────────────────────────────┘   │
│                              │                               │
│                              ▼                               │
│  ┌──────────────────────────────────────────────────────┐   │
...
| MCP 客户端 | `mcp/client.py` | 供 LangGraph/Pydantic AI 使用的 MCP 客户端 |
