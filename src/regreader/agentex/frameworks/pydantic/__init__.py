...
from ...shared import AgentMemory, StatusCallback, NullCallback
...
class PydanticAgent(BaseAgent):
    ...
    def _init_agent(self):
        ...
        try:
            ...
            raise ImportError(
                "Pydantic AI not installed. Run: pip install 'pydantic-ai>=1.0.0'"
            )
    ...
    async def chat(
        self, message: str, context: Context | None = None
    ) -> AgentResponse:
        """发送消息并获取响应"""
        ...
        try:
            async with self._agent.run_stream(message) as result:
                async for chunk in result.stream_text():
                    yield Event(EventType.TEXT_DELTA, data={"text": chunk})
        finally:
...
# 注册到工厂
from ..base import FrameworkFactory, FrameworkType
