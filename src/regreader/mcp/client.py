...
提供连接 RegReader MCP Server 的客户端功能。
...
class RegReaderMCPClient:
    ...
    async def _connect_stdio(self) -> None:
        """通过 stdio 传输连接（启动子进程）"""
        # RegReader MCP Server 启动参数（使用共享配置）
    ...
    async def __aenter__(self) -> "RegReaderMCPClient":
        """异步上下文管理器入口"""
        await self.connect()
        return self
