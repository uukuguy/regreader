...
MCP_SERVER_ARGS = ["-m", "grid_code.cli", "serve", "--transport", "stdio"]
...
class MCPConnectionConfig:
    ...
    def from_settings(cls) -> MCPConnectionConfig:
        ...
        读取 GridCodeSettings 中的 MCP 相关配置。
...
class MCPConnectionManager:
    ...
    def __init__(self, config: MCPConnectionConfig | None = None):
        ...
        self._client: GridCodeMCPClient | None = None
    ...
    async def get_client(self) -> GridCodeMCPClient:
        """获取已连接的 MCP 客户端

        如果客户端未创建或未连接，会自动创建并连接。

        Returns:
            已连接的 GridCodeMCPClient 实例
        """
        from grid_code.mcp.client import GridCodeMCPClient

        if self._client is None or not self._connected:
            self._client = GridCodeMCPClient(
                transport=self.config.transport,
                server_url=self.config.server_url,
            )
