...
工具集设计（精简后）：
- 核心工具（8个）：智能体检索安规必备，始终可用
- 高级分析工具（2个）：默认禁用，配置开关启用
"""
...
2. **get_toc(reg_id)** - 获取规程目录树
   - 用途：了解规程结构、确定搜索范围
   - 后续：smart_search, search_tables

3. **smart_search(query, reg_id, chapter_scope?, limit?, block_types?, section_number?)** - 混合检索
   - 用途：查找相关内容，结合关键词和语义检索
   - 前置：建议先用 get_toc 确定章节范围
   - 后续：read_page_range, lookup_annotation, resolve_reference

4. **read_page_range(reg_id, start_page, end_page)** - 读取页面内容
   - 用途：阅读完整页面、获取完整上下文
   - 前置：通常在 smart_search 后使用
   - 说明：自动处理跨页表格拼接，单次最多读取 10 页

## 多跳推理工具（3个）

5. **search_tables(query, reg_id, chapter_scope?, search_mode?, limit?)** - 搜索表格
   - 用途：查找特定表格、表格内容搜索
   - search_mode: keyword/semantic/hybrid（默认hybrid）
   - 后续：get_table_by_id, lookup_annotation

6. **lookup_annotation(reg_id, annotation_id, page_hint?)** - 追踪注释
   - 用途：查找「见注X」「方案A」等注释的完整内容
   - 支持变体匹配：注1/注①/注一、方案A/方案甲
   - page_hint 可加速搜索

7. **resolve_reference(reg_id, reference_text)** - 解析交叉引用
   - 用途：解析「见第X章」「参见表Y」等交叉引用
   - 后续：read_page_range（跳转阅读目标内容）

## 扩展工具（1个）

8. **get_table_by_id(reg_id, table_id, include_merged?)** - 获取完整表格
   - 用途：获取跨页表格的完整内容
   - 前置：search_tables（先搜索找到目标表格）
   - 后续：lookup_annotation（追踪表格中的注释引用）
...
### 简单查询流程
```
get_toc → smart_search(带chapter_scope) → read_page_range → 生成答案
```
...
4. read_page_range(reg_id, start_page, end_page) - 读取页面内容
5. search_tables(query, reg_id, chapter_scope?) - 搜索表格
6. lookup_annotation(reg_id, annotation_id, page_hint?) - 追踪注释
7. resolve_reference(reg_id, reference_text) - 解析交叉引用
8. get_table_by_id(reg_id, table_id) - 获取完整表格（含跨页合并）
...
# ==================== V2: 向后兼容别名 ====================
SYSTEM_PROMPT_V2 = SYSTEM_PROMPT_OPTIMIZED


# ==================== V3: 带领域知识的优化版 ====================
SYSTEM_PROMPT_V3 = """# 角色定义
你是电力系统安规专家助理 GridCode，帮助用户查找安规中的相关规定。
