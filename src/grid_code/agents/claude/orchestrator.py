...
协调多个 Claude Subagent 的执行，实现查询路由和结果聚合。
...
from grid_code.agents.claude.subagents import create_claude_subagent
...
class ClaudeOrchestrator(BaseGridCodeAgent):
    ...
    特性:
    - 上下文隔离：每个 Subagent 持有独立的 ClaudeSDKClient
    ...
    def _create_subagents(self) -> dict[SubagentType, BaseSubagent]:
        """创建 Subagent 实例"""
        subagents = {}
